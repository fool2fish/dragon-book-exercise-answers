...
消除立即左递归后为，注意该过程产生了产生式体有 ε 的产生式
...
对于 LR 来说，ε 会导致二义性问题。而消除 ε 以后的产生式，可能会有立即左递归，但也不会影响 LR，因为规约的时候看到的已经是完整的产生式体了。例如产生式 `A -> A a | b`，规约的时候，如果栈顶的符号为 a，就按 `A -> A a` 进行规约，如果栈顶的符号为 b，就按 `A -> b` 进行规约。
